...
# Scaling the lmssuggests service
echo "Running $1 instances of lmssuggests, each connecting to a different postgres DBMS"

if docker service ls --filter "name=lmssuggests" --format "{{.Name}}" | grep -q "^lmssuggests$"; then
  docker service scale lmssuggests=$1
else
  docker service create -d \
    --name lmssuggests \
    --env SPRING_PROFILES_ACTIVE=bootstrap \
    --env spring.datasource.url=jdbc:postgresql://suggests_db_{{.Task.Slot}}:5432/postgres \
    ...
    --mount type=bind,source=/c/user/gomes/SomeDirectory,target=/tmp/uploads-psoft-g1-instance{{.Task.Slot}} \
    --publish 8076:8080 \
    --network lms_overlay_attachable_network \
    lmssuggests:latest
