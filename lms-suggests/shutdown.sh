docker service scale lmssuggests=0
remove=$(docker service rm lmssuggests)

if [[ "$remove" == "lmssuggests" ]]; then
  echo "Stopped lmssuggests"
  ...
  echo "Could not stop lmssuggests"
fi