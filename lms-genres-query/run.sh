...
# Scaling the lmsgenresquery service
echo "Running $1 instances of lmsgenresquery, each connecting to a different postgres DBMS"

if docker service ls --filter "name=lmsgenresquery" --format "{{.Name}}" | grep -q "^lmsgenresquery$"; then
  docker service scale lmsgenresquery=$1
else
  docker service create -d \
    --name lmsgenresquery \
    --env SPRING_PROFILES_ACTIVE=bootstrap \
    --env spring.datasource.url=jdbc:postgresql://genres_query_db_{{.Task.Slot}}:5432/postgres \
    ...
    --mount type=bind,source=/c/Users/user/SomeDirectory,target=/tmp/uploads-psoft-g1-instance{{.Task.Slot}} \
    --publish 8055:8080 \
    --network lms_overlay_attachable_network \
    lmsgenresquery:latest
