...
# Scaling the lmsgenres service
echo "Running $1 instances of lmsgenres, each connecting to a different postgres DBMS"

if docker service ls --filter "name=lmsgenres" --format "{{.Name}}" | grep -q "^lmsgenres$"; then
  docker service scale lmsgenres=$1
else
  docker service create -d \
    --name lmsgenres \
    --env SPRING_PROFILES_ACTIVE=bootstrap \
    --env spring.datasource.url=jdbc:postgresql://genres_db_{{.Task.Slot}}:5432/postgres \
